...
class ListClassStudentAttendance extends React.Component {
  ...
  render() {
    const data = [
      ...
      {
        name: "Absent Without Valid Reason",
        value: this.state.overview.AbsentwithoutvalidReason,
      },
      {
        name: "Absent With Valid Reason",
        value: this.state.overview.AbsentwithvalidReason,
      },
    ];
    ...
    const absentee =
      this.state.overview.AbsentwithoutvalidReason +
      this.state.overview.AbsentwithvalidReason;
    ...
    let data1 = items.map((presents) => {
      return (
        <tr key={presents.id}>
          ...
          <td>
            {presents.firstName} {presents.lastName}
          </td>
          <td>{presents.userName}</td>
        </tr>
      );
    });
    ...
    let data2 = items1.map((absents) => {
      return (
        <tr key={absents.id}>
          ...
          <td>
            {absents.firstName} {absents.lastName}
          </td>
          <td>{absents.userName}</td>
          <td>
            {" "}
            <button className="btn btn-primary" onClick={this.viewLOA}>
              {" "}
              View LOA{" "}
            </button>
          </td>
    ...
    if (noofenrolment === 0) {
      return (
        <div>
          ...
          <div>
            {" "}
            No students found! Are you sure there are students being enrolled?
          </div>
        </div>
      );
    }
